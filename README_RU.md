...
Статус: **v2.1.0 одобрен пользователем 2026-09-06, без маппинга**. [Готовый комплект](https://github.com/Jestokyzz/mod-cultivation/releases/tag/v2.1.0), [установка и обновление](docs/install-v2.1.0.md), [изменения](docs/CHANGELOG_v2.1.0_RU.md), [модели и эффекты](docs/MODEL_PORTING_RU.md). Общие NPC, ресурсы и manifests находятся в корне модуля. Этот выпуск сам по себе не устанавливает обновление на основной сервер/клиент; проверенный источник — тестовый контур. Исторические инструкции ниже не заменяют руководство v2.1.0.
