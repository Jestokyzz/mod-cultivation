[CmdletBinding()]
param([string]$ServerRoot='C:\Solo WotLK\test-server\20260904-cultivation-v1')
...
foreach($file in @($world,$auth,$bots)){
    foreach($line in Get-Content -LiteralPath $file){
        if($line -match '^(LoginDatabaseInfo|WorldDatabaseInfo|CharacterDatabaseInfo|PlayerbotsDatabaseInfo)\s*=\s*"[^"]*;([^;"]+)"'){
            if($matches[2] -notmatch '^cultivation_test_(auth|world|characters|playerbots)_v1$'){throw "Non-isolated database in $file"}
        }
    }
}
...
foreach($port in @(3732,8099)){
    if(Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue){throw "Test port already occupied: $port"}
}
$logStamp=Get-Date -Format 'yyyyMMddTHHmmss'
$authProcess=Start-Process -FilePath (Join-Path $ServerRoot 'authserver.exe') -ArgumentList @('-c','configs/authserver.conf') -WorkingDirectory $ServerRoot -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $ServerRoot "logs\auth-stdout-$logStamp.log") -RedirectStandardError (Join-Path $ServerRoot "logs\auth-stderr-$logStamp.log")
$worldProcess=Start-Process -FilePath (Join-Path $ServerRoot 'worldserver.exe') -ArgumentList @('-c','configs/worldserver.conf') -WorkingDirectory $ServerRoot -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $ServerRoot "logs\world-stdout-$logStamp.log") -RedirectStandardError (Join-Path $ServerRoot "logs\world-stderr-$logStamp.log")
